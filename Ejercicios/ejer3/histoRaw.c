...
int main(int argc, char *argv[]){
   ...
   aux  = urnas;
   for(int i = 0; i<noUrnas ; i++){
      printf("%d - %d:\t\t %d\t\t ", i * tamUrnas, i*tamUrnas + tamUrnas, *aux);
      if(mayor == 1){
...
void procesaDir(DIR *dirptr, int *urnas){
   ...
   while(dirStruct != NULL){
      ...
      if(dirStruct->d_type == DT_REG){
         ...
         printf("Nombre archivo: %s\t", dirStruct->d_name);
         ...
         printf("urna antes: %d\t", *aux);
         *aux += 1;
         printf("urna despues: %d\n", *aux);
      }
         ...
         printf("Entrando a contenido de directorio: %s\n", nuevoDir);
